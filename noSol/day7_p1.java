...
int VM(String program, int[] input) {

    int[] intcode = Stream.of(program.split(",")).mapToInt(Integer::parseInt).toArray();
    int lastOutput = -1;
    int counter = 0;
    int ptr = 0;
    while(true) {
        ...
        switch(opcode) {
            ...
            case 2:
                ...
                break;
            case 3:
                intcode[intcode[ptr++]] = input[counter++];
                break;
            case 4:
                System.out.println(intcode[intcode[ptr]]);
                lastOutput = intcode[intcode[ptr++]];
                break;
            case 5:
...
private static void permutation(String prefix, String str) {
    int n = str.length();
    if (n == 0) permutations[ptr++] = Stream.of(prefix.split("")).mapToInt(Integer::parseInt).toArray();;
    else {
        for (int i = 0; i < n; i++)
...
for(int i = 0; i < permutations.length; i++) {
    for(int j = 0; j < AMPLIFIERS; j++) {
        outPutSignal = VM(puzzleInput, new int[]{permutations[i][j], outPutSignal});
    }
    MAX_VAL = Math.max(MAX_VAL, outPutSignal);
}
