...
class Point {
    ...
    Point(int x, int y) {
        this.x = x;
        this.y = y;
        manhattandist = Math.abs(x) + Math.abs(y);
    }
    Point addVector(int x, int y) {
...
class Line {
    ...
    Line(Point p1, Point p2) {
        this.p1 = p1;
        this.p2 = p2;
        plane = p1.x == p2.x? Plane.VERTICAL: Plane.HORIZONTAL;
    }

    Point[] getIntersections(Line l2) {
        ...
        if(plane != l2.plane) {
            if(plane == Plane.VERTICAL) {
                int x = p1.x;
                int y = l2.p1.y;
                if(x >= l2.p1.x && x <= l2.p2.x && y >= p1.y && y <= p2.y)
                    intersections.add(new Point(x, y));
            }
            if(plane == Plane.HORIZONTAL) {
                int x = l2.p1.x;
                int y = p1.y;
                if(x >= p1.x && x <= p2.x && y >= l2.p1.y && y <= l2.p2.y)
                    intersections.add(new Point(x, y));
            }
        }
        else if(plane == Plane.HORIZONTAL && p1.y == l2.p2.y) {
            int[] xVals = new int[4];
            xVals[0] = p1.x; xVals[1] = p2.x; xVals[2] = l2.p1.x; xVals[3] = l2.p2.x;
            Arrays.sort(xVals);
            int start = xVals[1];
            int end = xVals[2];

            for(int x = start; x <= end; x++)
                intersections.add(new Point(x, p1.y));
        }
        else if(plane == Plane.VERTICAL && p1.x == l2.p2.x) {
            int[] yVals = new int[4];
            yVals[0] = p1.y; yVals[1] = p2.y; yVals[2] = l2.p1.y; yVals[3] = l2.p2.y;
            Arrays.sort(yVals);
            int start = yVals[1];
            int end = yVals[2];

            for(int y = start; y <= end; y++)
                intersections.add(new Point(p1.x, y));
        }
        Object[] tmp = intersections.toArray();
        Point[] res = new Point[tmp.length];
        for(int i = 0; i < tmp.length; i++)
            res[i] = (Point)tmp[i];
        return res;
    }
    
    static Line[] intoLines(String[] strWires) {
        Line[] wires = new Line[strWires.length];

        Point actualPt = new Point(0, 0); 
        Point nextPt = null;
        for(int i = 0; i < wires.length; i++) {
            String dir = strWires[i].substring(0, 1);
            int dist = Integer.parseInt(strWires[i].substring(1));
            
            switch(dir) {
                case "U":
                    nextPt = actualPt.addVector(0, dist);
                    break;
                case "D":
                    nextPt = actualPt.addVector(0, -dist);
                    break;
                case "L":
                    nextPt = actualPt.addVector(-dist, 0);
                    break;
                case "R":
                    nextPt = actualPt.addVector(dist, 0);
                    break;
            }  
            wires[i] = new Line(actualPt, nextPt);
            actualPt = nextPt;
        }
        return wires;
    }
...
for(Line l1: wire1) {
    for(Line l2: wire2) {
        ...
        for(Point p: intersections) {
            System.out.println("Intersection found at Point(" + p.x + "|" + p.y);
            distances.add(p.manhattandist);
        }
    }
}